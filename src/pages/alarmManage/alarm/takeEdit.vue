        ...
        <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="120">
            <div class="c-top-border-gray">
                <div class="c-form-row-1col">
                    <FormItem label="报警名称：">
                        <span v-if="(selectedData.length==0)&&(!alarmName)">请选择</span>
...
<script>
// 
import { alarmList ,getOrg ,getTree,getUsers,subDetail,editSub} from '@/api/alarm/subscribe'
...
export default {
    ...
    methods: {
        getDetail(id){
            subDetail(id).then(res=>{
                console.log(res)
                let temp = res.data.subscribeList[0]
                this.paramList =res.data.subscribeList 
                this.areaName = res.data.siteName
                this.alarmName = res.data.defineName
                this.formValidate = {
                    name: res.data.defineName,
                    frequency: temp.pushFrequency,
                    delayTime: temp.delayPushTime,
                    way: temp.subscribeMode.split(','),
                    single: temp.pushReleaseMessage=='Y'?true:false,
                    organize: '',
                    user: [temp.userId],
                    siteId:res.data.siteId,
                    siteName : res.data.siteName,
                    defineId: res.data.defineId
                }
                this.receiveObj = [{
                    org:'',
                    user:[
        ...
        selectUser(user){
            ...
            this.formValidate.user.push(user.value)
        ...
        saveHandle(){
            console.log(this.formValidate.user)
            let data = {
                defineId:this.formValidate.defineId,
                defineIds: this.formValidate.defineId,
                defineName: this.formValidate.name,
                ...
                siteIds: this.formValidate.siteId,
                siteId: this.formValidate.siteId,
                siteName: this.formValidate.siteName,
                ...
                subscribeList:this.paramList
            }
            console.log(data)
            editSub(data).then(res=>{
                console.log(res)
                if(res.data.ids){
