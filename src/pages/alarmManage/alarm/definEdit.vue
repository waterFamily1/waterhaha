...
<script>
import {getUsers,getTree,getModalList,editDefinition,getDetail} from '@/api/alarm/definition'
...
export default {
            ...
            columns1: [
                {
                    ...
                    render: (h, params) => {
                        ...
                        if(this.data1[params.index].symbol =='5分钟无数据'){
                            ...
                            props:{
                                size:'small',
                                value:that.data1[params.index].threshold
                            },
                            on:{
                                    input(val){
                    ...
                    render: (h,params) => {
                        ...
                        return h('Select',{
                            props:{
                                size:'small',
                                transfer:true,
                                value :that.data2[params.index].userId
                            },
                            on:{
                                'on-change':(val)=>{
                    ...
                    render: (h,params) => {
                        ...
                        return h('Select',{
                            props:{
                                size:'small',
                                transfer:true,
                                value :that.data2[params.index].subscribeMode,
                                value:'',
                                multiple:true,
                            },
                            on:{
                        ...
                        return h('div', [
                            h('Input', {
                                props: {
                                 ...
                                 on:{
                                input(val){
                                   that.data3[0].processingPlan = val
                                }
                            }
                            })
                        ]);
                   ...
                   render: (h,params) => {
                        ...
                        return h('Select',{
                            props:{
                                size:'small',
                                transfer:true,
                                value :this.data3[0].contactPersonId
                            },
                            on:{
                                'on-change':(val)=>{
                                   console.log(val)
                                   this.receiveObj.map(ele=>{
                                       if(ele.value == val){
                                           that.data3[0].contactPersonName = ele.label
                                       }
                                   })
                                   
                                   this.data3[0].contactPersonId= val
                                }
                            }
                        },
                        this.receiveObj.map(item=>{
    ...
    methods: {
                ...
                if(res.data){
                    ...
                    console.log( temp.alarmName)
                     this.formValidate= {
                        area: temp.siteId,
                        name: temp.alarmName,
                        confirmWay: temp.disarmMethod,
                        time: temp.alarmWaitTime,
                    ...
                    this.data1Show = temp.conditions
                    this.tempArr = temp.conditions
                    this.data2 = temp.subscribeList
                    this.data2Show = temp.subscribeList
                    this.contactPhone = temp.contactPhone
                    this.data2.map(item=>{
                        item.subscribeMode = item.subscribeMode.split(',')
                    })
                    console.log(this.data2)
                    this.data3[0].processingPlan = temp.processingPlan
                    this.data3[0].contactPersonName = temp.contactPersonName
                    this.data3[0].contactPersonId = temp.contactPersonId
             ...
             this.$refs[name].validate((valid) => {
                if (valid) {
                    ...
                    this.tempArr.filter((item,index)=>{
                        ...
                        // if((item.symbol!='5分钟无数据')&&(!item.threshold)){
                    ...
                    console.log(this.data2)
                    this.data2.map(item=>{
                        item.pushReleaseMessage = this.single?'Y':'N'
                    })
                    let data = {
                        ...
                        contactPersonId: this.data3[0].contactPersonId,
                        contactPhone:this.contactPhone,
                        disarmMethod: this.formValidate.confirmWay,
                        enabledStatus: this.formValidate.state,
                        id:this.id,
                        processingPlan: this.data3[0].processingPlan,
        ...
        deleteIdx(index){ 
            ...
            this.data1Show.splice(index,1)
            this.tempArr[index].status = 'delete'
        },
        addDefine(){
            this.data2Show.push({
                subscribeMode:'',
        ...
        deleteDef(index){
            this.data2Show.splice(index,1)
            this.data2[index].status = 'delete'
        }
    }
}
</script>
