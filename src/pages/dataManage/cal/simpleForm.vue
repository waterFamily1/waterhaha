                    ...
                    <Row style="padding-left:20px;padding-top: 20px;">
                        <Col span="8">
                            <FormItem label="任务名称:" prop="taskname">
                                <Input v-model="point.taskname" :disabled="isAutoSetName" placeholder="任务名称"></Input>
                            </FormItem>
                            <Checkbox style="position:absolute;top:8px;right:-70px;" v-model="isAutoSetName" :disabled="editAutoNameDisabled" @on-change="boxChange">自动</Checkbox>
                        </Col>
                        <Col span="8" offset="3">
                            <FormItem label="数据源测点:" prop="mpointname"
                            :rules="[{ required: true, message: '请选择数据源测点' }]">
                                <div @click="dmpointClick">
                                    <Input v-model="point.mpointname" placeholder="测点" :disabled="disabledForSite" readonly></Input>
                                </div>
                            </FormItem>
                        </Col>
                    </Row>
                    <Row style="padding-left:20px;">
                        <Col span="8">
                            <FormItem label="计算方法:" prop="formulaid">
                                <Select v-model="point.formulaid" @on-change="changeFormula" :disabled="$route.query.id ? 'disabled': !dmpoint.id" :label-in-value="true">
                        ...
                        <Col span="8" offset="3">
                            <FormItem label="计算周期:" prop="cycle">
                                <Select v-model="point.cycle" @on-change="changeCycle" :disabled="$route.query.id ? 'disabled': !dmpoint.id">
                    ...
                    <Row style="padding-left:20px;">
                        ...
                        <Col span="8" offset="3">
                            <FormItem label="周期开始时间" prop="cycle">
                                <Select v-model="point.cycleDt" :disabled="disabledForSite || point.cycle == 1" ref="resetSelect">
...
<script>
import { areaMethod, dataMethod, fromEqu, countMethod, countCycleMethod, dataMethod1, saveMethod, editMethod, editMethod1, editMethod2, saveMethod1 } from '@/api/dataManage/cal'
...
export default {
    data() {
        return {
            ...
            ruleValidate: {
                ...
                mpointname: [
                    { required: true, message: '请选择数据源测点' },
                    {
                        validator (rule, value, callback) {
                            if(value != '') { callback(); }
                        }
    ...
    methods: {
        getEdit() {
            ...
            editMethod1(this.$route.query.id).then(res=> {
                ...
                editMethod2(res.data.items.id).then(res=> {
                    this.point.taskname = res.data.items[0].mpointName+'_'+this.formulaName
                    this.point.mpointname = res.data.items[0].mpointName
                    this.dmpoint.id = res.data.items[0].mpointid
                })
            })
        },
        getArea() {
        ...
        changeFormula(item) {
            ...
            this.point.taskname = this.point.mpointname+'_'+this.formulaName
        },
        addSearch1() {
        ...
        chooseHandle1(row) {
            this.point.mpointname = row.mpointName
            this.dmpoint.id = row.id
            this.mpointModal = false
            if(this.point.formulaid == '' || this.point.formulaid == undefined || this.point.formulaid == null) {
                this.point.taskname = row.mpointName
            } else {
                this.point.taskname = row.mpointName+'_'+this.formulaName
            }
        },
        changePage1(num) {
            this.pageNum1 = index
