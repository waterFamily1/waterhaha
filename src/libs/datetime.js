const DT = {
	...
	midnightEnd(d) {
		if (DT.F(d, 'yyyy-MM-dd')==DT.F(new Date, 'yyyy-MM-dd')){
			return DT.F(new Date, 'yyyy-MM-dd hh:mm:ss');
		}
		return DT.F(d)+' 23:59:59';
	},
	midnightMill(d) {
	...
	getPeriodsNew (params) {
		var start = params.beginDate,
			baseDate = new Date(DT.formatePolyfill(start)),
			baseMills = baseDate.getTime(),
			end = params.endDate,
			endDate = new Date(DT.formatePolyfill(end)),
